...
export default function Pickup({
	...
	tiers }: {
		...
		allOverviewArchives: AllOverviewArchives[]
		currentPaths: string[],
	...
	const pickupOverviewArchives = allOverviewArchives.map(arc => {
				...
				{!isVideoMode &&
					<Flex flexGrow={1} direction='row' bg={bgColor}>
						<Grid templateColumns={{ base: '1fr', lg: '240px 1fr', xl: '300px 1fr' }} >
							...
							<VStack spacing={8} px={{ base: 4, md: 8 }} py={{ base: 8, md: 8 }} overflow='hidden'>
								...
								<>
									<VStack w='full' spacing={20} align='flex-start'>
										<>
											<Box>{'All'}</Box>
											<Box>{'最新'}</Box>
								...
								<>
									<VStack w='full' spacing={20} align='flex-start'>
										{pickupOverviewArchives.map((arc, i) =>
											<>
												<Box>{arc.categoryName[locale]}</Box>
												<Box>{arc.paths.name[locale]}</Box>
...
export const getStaticProps: GetStaticProps = async ({ locale, params }) => {
	...
	const overviewArchives = await items[0].archiveRouteArray
		.filter(obj => obj.id !== 'all' && obj.id !== 'favorite')

	const paths = overviewArchives.map(obj => (obj.paths))
	...
	const allOverviewArchives = overviewArchives.map((arc, i) => ({ ...arc, paths: pathAndArchives[i] }))
