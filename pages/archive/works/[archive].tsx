...
import { VStack, Box, List, ListItem, Link, useColorModeValue, Tabs, TabList, TabPanels, Tab, TabPanel, Accordion, AccordionItem, AccordionButton, AccordionPanel, AccordionIcon, useToast } from '@chakra-ui/react'
		...
		<>
			<VStack w='full' maxW={{ base: '1000px' }} py={{ base: 12, lg: 24 }} spacing={24} margin='0 auto'>
				<Tabs w='full' id={`archive-tab-id`} isFitted>
					...
					<TabPanels>
						<TabPanel p={0}>
							<VStack w='full' spacing={archive?.youtubeId ? { base: 0, lg: 12 } : 0} pt={archive?.learningVideoId ? { base: 10, md: 20 } : 0}>
								<VStack w='full' spacing={archive?.timestamp ? 6 : 0}>
									{archive?.youtubeId && <VideoYouTube
										...
										onRouterPush={null} />}
									{archive?.timestamp &&
										<Accordion w='full' mt='6px!important' css={accordionCss}
